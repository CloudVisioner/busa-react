...
export function WhatIsBusa({ className }: WhatIsBusaProps) {
  return (
    <section
      id="about"
      className={cn(
        'relative bg-surface py-12 md:bg-[radial-gradient(circle_at_top_left,rgba(0,35,111,0.04),transparent_50%)] md:py-32',
        className
      )}
    >
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-6 py-12 md:px-8 md:py-0 lg:grid-cols-[1fr_1.1fr] lg:gap-16">
        <div className="flex flex-col gap-8 lg:pr-6">
          <span className="-ml-1 inline-flex w-fit rounded-full bg-primary/10 px-5 py-2.5 text-sm font-extrabold uppercase tracking-[0.16em] text-primary md:ml-0 md:text-base">
            Biz haqimizda
          </span>

          <div>
            <h2 className="font-headline text-4xl font-bold leading-[1.05] tracking-tight text-primary md:text-6xl">
              Koreyadagi o&apos;zbek talabalar hamjamiyati.
            </h2>
            <div className="mt-6 h-1 w-16 rounded-full bg-primary" />
          </div>

          <p className="max-w-xl text-base leading-7 text-on-surface-variant md:text-xl">
          ...
          <Link
            className="group mt-2 inline-flex self-start items-center gap-2 text-base font-bold text-primary underline decoration-primary/40 underline-offset-4"
            href={ROUTES.ABOUT}
          >
